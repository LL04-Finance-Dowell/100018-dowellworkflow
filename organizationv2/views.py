...
from editor.report_connection import save_workflow_report
...
from .mail import formated_invitation_mail
...
class CreateOrganizationv2(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        if company :
            ...
            return render(request, 'organizationv2/create.html', { 'form': form, 'all_members': all_members, 'company': company })
...
def formate_mail(email_id, company_name, link, admin):
    return f'''
    Dear {email_id},
    You are invited to join {company_name.title()} for digital documentations.
    Kindly click on the link to accept the invitation:
    {link}

    Login to the account using the provided credentials and get back to us if you are facing any issues:
    Username : {email_id.split('@')[0]}
    Password: password123Dowell

    Kindly change the reset the password after you logged in successfully.
...
def add_member(request, company_id):
    if request.method == 'POST':
        ...
        if email != '':
            ...
            #   mail_content = formate_mail(email, company.company_name, link, company.admin.username)
            #   send_mail('Invitaion from DocEdit', mail_content,'', [token_entry.user_email,], fail_silently=False)
            #   return JsonResponse({ 'status': 'OK', 'message': 'Email sent at ' + token_entry.user_email, 'link': link})

            msg = formated_invitation_mail(email, company.company_name, link, company.admin.username)
            send_mail('You are invited at DocEdit','','',[token_entry.user_email,],fail_silently=False, html_message=msg)
...
class CreateProjectView(View):
    ...
    def get(self, request, *args, **kwargs):
        ...
        return render(request, 'organizationv2/create_project.html', { 'org': org, 'form': form, 'all_members': all_members, 'company':company })
        ...
        if (request.user == proj.project_lead) :
            ...
            for usr in list(proj.members.all()):
                if usr not in userlist:
                    userlist.append(usr)
    userlist.append(org.company.admin)

    print('USER_LIST :', userlist)
    context = {
        'object_list': org.workflows.all(),
        'user_list': list(set(userlist)),
...
def create_document_type(request, *args, **kwargs):
	...
	org.save()
	report = save_workflow_report(obj, request.user)
...
from editor.report_connection import save_template_entry
...
def create_template(request, *args, **kwargs):
    ...
    if request.method == 'POST':
        ...
        if form.is_valid() :
            ...
            resObj = save_template_entry(template);
            print('This is a response from Report server', resObj)
            return redirect('editor:template-editor', company_id=org.company.id, temp_id=template.id)


    CreateTemplateForm.base_fields['document_type'] = forms.ModelChoiceField(queryset=org.workflows.all())
    CreateTemplateForm.base_fields['copy_template'] = forms.ModelChoiceField(required=False, queryset=org.templates.all())
    ...
    print('ORG GETTING COMPANY ID', org.company)
    return render(request, 'doc_template/create_template.html', context=context)


def create_project_template(request, *args, **kwargs):
    org = get_object_or_404(Organizationv2, id=kwargs['org_id'])
...
class CreateTemplate(View):
    ...
    def post(self, request, *args, **kwargs):
        ...
        if form.is_valid() :
            ...
            template.save()
            org.templates.add(template)
            #return redi('editor:template-editor', id=template.id)
