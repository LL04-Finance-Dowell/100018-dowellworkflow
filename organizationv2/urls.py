...
from .views import add_member_to_project, add_member_to_organization, create_project_template
...
urlpatterns = [
    ...
    path('<int:org_id>/<int:project_id>/create-template', login_required(create_project_template), name="create-project-template"),
    path('<int:org_id>/<int:project_id>/previous-template', login_required(previous_templates), name="previous-project-templates"),


    #path('<int:org_id>/get-pdf/', login_required(views.get_pdf), name="get_pdf"),
    #path('<int:org_id>/get-pdf-json/', login_required(views.get_pdf_json), name="get_pdf_json"),



    #include(('app_name.urls', 'app_name'), namespace='app_name')
