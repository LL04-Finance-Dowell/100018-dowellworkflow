...
def get_pdf_json(request):
    ...
    for signed_file in signed_files:
        ...
        with open("E:/Dowell/media/files/HT5VUOC69E.json") as f:
            ...
            for component in data_s:
                ...
                left = component['left']
                type = component['type']
                auth_users = component['auth_user']
                content = component['data']
                lines += [width, height, top, left, type, auth_users, content]
    print(lines)
    pdf = canvas.Canvas("Signed_Documents.pdf")
    pdf.setTitle("Signed Documents Summary")
    # Set Title
    for font in pdf.getAvailableFonts():
        print(font)
    pdf.drawString(220, 750, "Signed Document Report")
    # subtitle
    pdf.setFillColorRGB(0, 0, 255)
    pdf.setFont("Courier-Bold", 24)
    pdf.drawString(100, 720, "This is Test Subtitle")
    # draw a line
    ...
    pdf.drawText(text)
    # Draw an Image
    image = "https://comparables.co.ke//static/blog/location1.gif"
    pdf.drawInlineImage(image, 50, 200)
